...
def samples(group='policy.memory'):
    ...
    derivative_config = value.TaskValueConfig(window_steps=2, batch_size=8,
        derivative_state_group=group, derivative_samples=2, derivative_holdout_samples=2,
...
def test_true_labels_match_frozen_actor_continuation_and_fixed_coordinates(group, scale):
    ...
    for batch in (train, heldout):
        for i, step in enumerate(batch.steps.tolist()):
            ...
            torch.testing.assert_close(batch.gradients[i:i+1], expected, rtol=1e-9, atol=1e-10)
        assert not batch.gradients.requires_grad
        ...
        for i, step in enumerate(batch.steps.tolist()):
            ...
            torch.testing.assert_close(predicted[i:i+1], expected, rtol=1e-9, atol=1e-10)
...
def test_derivative_fit_preserves_actor_calibrates_once_and_restores_weights():
    ...
    norms = weights['initial_gradient_norms']
    ...
    stale = copy.deepcopy(trainer.state_dict()); stale['objective']='task-value-v1-exact-step-cost-suffix'
...
def test_derivative_configuration_routes_explicit_sampling_and_new_objective():
    ...
    args = parse_args(['--optimizer', 'task-adam', '--scenario-mode', 'fixed-airframe',
        '--value-derivative-state-group', 'policy.memory', '--value-derivative-boundaries', '50', '250', '450',
        '--value-derivative-samples', '24', '--value-derivative-holdout-samples', '12',
        '--value-derivative-batch-size', '32'])
    config = critic_configuration(args)
    assert config.sampling_boundaries(500) == (50,250,450)
    assert config.derivative_samples == 24 and config.derivative_holdout_samples == 12

"""Historical finite-difference diagnostics; never imported by normal training."""
...
import torch
from response_proposals import gradient_basis, assign_parameters
...
def check_direction_at_scales(full: torch.Tensor, half: torch.Tensor, q: torch.Tensor,
                              config: FiniteDifferenceConfig) -> tuple[bool, dict]:
...
@torch.no_grad()
def correct_direction(policy, gradients, evaluate_train, config: FiniteDifferenceConfig, *, baseline=None):

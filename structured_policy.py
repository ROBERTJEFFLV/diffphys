    ...
    def forward_with_aux(self, observation: torch.Tensor,
        ...
        if update_slow:
            ...
            if state.slow_counter == self.config.identification_publish_start:
                # The first measured window initializes the estimate. Zero
                # before publication is absence of information, not a force
                # measurement to blend with the first valid estimate.
                observer_gain = 1.0
        ...
        nominal_action_cap = action_cap
        # An inherited/external command may already be outside the trim box.
        # Include that executed starting point, without allowing its maximum
        # deviation from trim to increase. The physical box and slew bound
        # remain hard constraints. This same recovery rule runs at deployment.
        inherited_deviation = (previous_action - equilibrium_trim).abs().amax(-1, keepdim=True)
        action_cap = torch.maximum(action_cap, inherited_deviation)
        ...
        return StructuredPolicyOutput(action, next_state, {
            ...
            "nominal_action_delta_cap": nominal_action_cap.squeeze(-1),
            "inherited_command_recovery": (action_cap > nominal_action_cap).squeeze(-1),

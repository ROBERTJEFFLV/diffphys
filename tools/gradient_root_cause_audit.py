...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--state-step-grad-decay", type=float, default=None)
    parser.add_argument("--hidden-step-grad-decay", type=float, default=None)
...
def load_policy(path: Path, device: torch.device) -> tuple[MotorGRUPolicy, dict, bool]:
    checkpoint = torch.load(path, map_location=device)
    state_dict = checkpoint.get("model", checkpoint)
    source_dim = int(state_dict["encoder.0.weight"].shape[1])
    if source_dim != 40:
        raise RuntimeError(
            "cuda-full gradient audit only supports legacy40 observations; "
            f"checkpoint uses {source_dim}D"
        )
    policy = MotorGRUPolicy(observation_dim=40).to(device=device)
    missing, _ = policy.load_compatible_state_dict(state_dict)
    return policy, checkpoint if isinstance(checkpoint, dict) else {}, bool(missing)
...
def main() -> None:
    ...
    def _flag_names(argv: list[str]) -> set[str]:
        return {arg.split("=", maxsplit=1)[0] for arg in argv if arg.startswith("--")}
    ...
    arg_names = _flag_names(sys.argv[1:])

    params = L2FParams()
    loss_config = L2FLossConfig()
    sim = L2FSimulator(params)
    policy, checkpoint, action_only_checkpoint = load_policy(Path(args.checkpoint), device)
    optimizer = torch.optim.AdamW(policy.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    if not args.ignore_optimizer_state and "optimizer" in checkpoint and not action_only_checkpoint:
        optimizer.load_state_dict(checkpoint["optimizer"])
        optimizer_to_device(optimizer, device)

    checkpoint_args = checkpoint.get("args", {}) if isinstance(checkpoint, dict) else {}
    state_grad_decay = args.state_grad_decay
    hidden_grad_decay = args.hidden_grad_decay
    dt = args.dt
    if isinstance(checkpoint_args, dict):
        if "--dt" not in arg_names and "dt" in checkpoint_args:
            dt = _coerce_float(checkpoint_args["dt"], dt)
        if "--state-grad-decay" not in arg_names and "state_grad_decay" in checkpoint_args:
            state_grad_decay = _coerce_float(checkpoint_args["state_grad_decay"], state_grad_decay)
        if "--hidden-grad-decay" not in arg_names and "hidden_grad_decay" in checkpoint_args:
            hidden_grad_decay = _coerce_float(checkpoint_args["hidden_grad_decay"], hidden_grad_decay)

    state_step_decay = (
        args.state_step_grad_decay
        if args.state_step_grad_decay is not None
        else state_grad_decay ** dt
    )
    hidden_step_decay = (
        args.hidden_step_grad_decay
        if args.hidden_step_grad_decay is not None
        else hidden_grad_decay ** dt
    )

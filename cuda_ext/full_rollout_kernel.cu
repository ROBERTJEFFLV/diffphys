...
__device__ inline void matvec3(const float a[9], const float x[3], float out[3]) {
    for (int r = 0; r < 3; r++) {
        out[r] = a[r * 3 + 0] * x[0] + a[r * 3 + 1] * x[1] + a[r * 3 + 2] * x[2];
    }
}

__device__ inline void matTvec3(const float a[9], const float x[3], float out[3]) {
    for (int c = 0; c < 3; c++) {
        out[c] = a[0 * 3 + c] * x[0] + a[1 * 3 + c] * x[1] + a[2 * 3 + c] * x[2];
    }
}
...
__device__ inline void solve3x3(const float a[9], const float b[3], float x[3]) {
    const float det =
        ...
        a[1] * (a[3] * a[8] - a[5] * a[6]) +
        a[2] * (a[3] * a[7] - a[4] * a[6]);
    const float det_safe = fabsf(det) < 1.0e-12f ? copysignf(1.0e-12f, det == 0.0f ? 1.0f : det) : det;
    x[0] = (
        b[0] * (a[4] * a[8] - a[5] * a[7]) -
        a[1] * (b[1] * a[8] - a[5] * b[2]) +
        a[2] * (b[1] * a[7] - a[4] * b[2])
...
__device__ inline void implicit_midpoint_jacobian3(
    ...
    for (int r = 0; r < 3; r++) {
        for (int c = 0; c < 3; c++) {
            out[r * 3 + c] = (r == c ? 1.0f : 0.0f) + 0.5f * dt * cjac[r * 3 + c] / inertia[r];
        }
    }
}
...
__device__ inline void implicit_midpoint_residual3(
    ...
    for (int i = 0; i < 3; i++) {
        residual[i] = omega_next[i] - omega[i] - dt * ((torque[i] - gyro_mid[i]) / inertia[i]);
    }
}
...
__device__ float potential_and_components(
    ...
    const LossParams lp,
    float* comps) {
    float pos = 0.0f, vel = 0.0f, omg = 0.0f;
    for (int i = 0; i < 3; i++) {
...
__global__ void physics_forward_kernel(
    ...
    implicit_midpoint_omega3(omega, torque, inertia, dp.dt, nw);
    for (int i = 0; i < 3; i++) {
        w[next3 + i] = nw[i];
    }
    const float omega_mid[3] = {
    ...
    const float phi[3] = {dp.dt * omega_mid[0], dp.dt * omega_mid[1], dp.dt * omega_mid[2]};
...
__global__ void potential_kernel(
    ...
    potentials[idx] = potential_and_components(
        &p[idx * 3], &v[idx * 3], &w[idx * 3], lp, comps);
}
...
__global__ void loss_adjoint_kernel(
    ...
    float comps[3];
    const float V = potential_and_components(&p[s3], &v[s3], &w[s3], lp, comps);
    if (lp.terminal_loss_only) {
        if (state_idx == horizon) {
            const float inv_b = 1.0f / static_cast<float>(batch);
            atomicAdd(&metrics[M_TRACKING], V * inv_b);
            atomicAdd(&metrics[M_POSITION], comps[0] * inv_b);
            atomicAdd(&metrics[M_VELOCITY], comps[1] * inv_b);
            atomicAdd(&metrics[M_OMEGA], comps[2] * inv_b);
            atomicAdd(&metrics[M_LOSS], V * inv_b);
            add_potential_grad(
                &p[s3], &v[s3], &w[s3], lp, inv_b,
                &lp_adj[s3], &lv_adj[s3], &lw_adj[s3]);
        }
        return;
    }
    ...
    atomicAdd(&metrics[M_OMEGA], comps[2] * inv_bh);
...
__global__ void physics_backward_kernel(
    ...
    float implicit_jacobian[9], implicit_jacobian_t[9], lambda[3], inv_inertia_lambda[3], gyro_jacobian[9], gyro_term[3], gtorque[3];
    implicit_midpoint_jacobian3(omega_mid, inertia, dp.dt, implicit_jacobian);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        gtorque[i] = dp.dt * inv_inertia_lambda[i];
    }
    matTvec3(gyro_jacobian, inv_inertia_lambda, gyro_term);
    for (int i = 0; i < 3; i++) {
        gw_in[i] += lambda[i] - 0.5f * dp.dt * gyro_term[i];
    }
...
__global__ void actor_backward_kernel(
    ...
    float* action_adj,
    float* hidden_adj,
    float* hidden_adj_before_mag,
    float* hidden_adj_after_mag,
    ...
    __shared__ float hidden_reduce[256];
    ...
    if (hidden_adj_before_mag != nullptr) {
        for (int h = tid; h < HID_DIM; h += blockDim.x) {
            const float candidate = fabsf(hidden_adj[(t * batch + b) * HID_DIM + h]);
            if (candidate > max_hidden) {
                max_hidden = candidate;
            }
        }
        hidden_reduce[tid] = max_hidden;
        __syncthreads();
        for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
            if (tid < stride) {
                const float other = hidden_reduce[tid + stride];
                if (other > hidden_reduce[tid]) {
                    hidden_reduce[tid] = other;
                }
            }
            __syncthreads();
        }
        if (tid == 0) {
            hidden_adj_before_mag[t * batch + b] = hidden_reduce[0];
        }
        __syncthreads();
    }
    ...
    if (hidden_adj_after_mag != nullptr) {
        max_hidden = 0.0f;
        for (int h = tid; h < HID_DIM; h += blockDim.x) {
            const float candidate = fabsf(hidden_adj[(t * batch + b) * HID_DIM + h]);
            if (candidate > max_hidden) {
        ...
        if (tid == 0) {
            hidden_adj_after_mag[t * batch + b] = hidden_reduce[0];
        }
        __syncthreads();
    }
...
std::vector<torch::Tensor> l2f_full_rollout_cuda(
    ...
    auto p = at::empty({states, batch, 3}, opts);
    auto v = at::empty({states, batch, 3}, opts);
    auto R = at::empty({states, batch, 3, 3}, opts);
    auto w = at::empty({states, batch, 3}, opts);
    auto m = at::empty({states, batch, 4}, opts);
    auto pa = at::empty({states, batch, 4}, opts);
    auto hidden = at::empty({states, batch, HID_DIM}, opts);
    auto enc0 = at::empty({horizon, batch, ENC_DIM}, opts);
    auto enc1 = at::empty({horizon, batch, ENC_DIM}, opts);
    auto gate_i = at::empty({horizon, batch, GATE_DIM}, opts);
    auto gate_h = at::empty({horizon, batch, GATE_DIM}, opts);
    auto actions = at::empty({horizon, batch, ACT_DIM}, opts);
    auto potentials = at::empty({states, batch}, opts);

    auto lp_adj = at::zeros({states, batch, 3}, opts);
    auto lv_adj = at::zeros({states, batch, 3}, opts);
    auto lR_adj = at::zeros({states, batch, 3, 3}, opts);
    auto lw_adj = at::zeros({states, batch, 3}, opts);
    auto lm_adj = at::zeros({states, batch, 4}, opts);
    auto lpa_adj = at::zeros({states, batch, 4}, opts);
    auto action_adj = at::zeros({horizon, batch, ACT_DIM}, opts);
    auto hidden_adj = at::zeros({states, batch, HID_DIM}, opts);
    auto hidden_adj_before_mag = collect_debug
        ? at::zeros({states, batch}, opts)
        : torch::Tensor();
    auto hidden_adj_after_mag = collect_debug
        ? at::zeros({states, batch}, opts)
        : torch::Tensor();
    auto metrics = at::zeros({METRIC_DIM}, opts);

    auto g_encoder0_w = at::zeros_like(encoder0_w);
    auto g_encoder0_b = at::zeros_like(encoder0_b);
    auto g_encoder1_w = at::zeros_like(encoder1_w);
    auto g_encoder1_b = at::zeros_like(encoder1_b);
    auto g_gru_w_ih = at::zeros_like(gru_w_ih);
    auto g_gru_w_hh = at::zeros_like(gru_w_hh);
    auto g_gru_b_ih = at::zeros_like(gru_b_ih);
    auto g_gru_b_hh = at::zeros_like(gru_b_hh);
    auto g_head_w = at::zeros_like(motor_head_w);
    auto g_head_b = at::zeros_like(motor_head_b);
    ...
    for (int rev = 0; rev < horizon; rev++) {
        ...
        actor_backward_kernel<<<batch, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            collect_debug ? hidden_adj_before_mag.data_ptr<float>() : nullptr,
            collect_debug ? hidden_adj_after_mag.data_ptr<float>() : nullptr,
